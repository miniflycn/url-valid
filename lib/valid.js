...
module.exports = (function () {
  'use strict';
  var deprecate = require('depd')('url-valid')
    , http = require('http')
    , https = require('https')
    ...
    if (match) {
      ...
      req = httpLib.request(opts, function (res) {
        ...
        if (statusCode === 200) {
          ...
          req.abort();
          var valid = one(URL.resolve(url, res.headers.location), function (err, valid) {
  ...
  Valid.one = deprecate.function(one, '.one will be removed in url-valid 1.0, just use valid instead');
